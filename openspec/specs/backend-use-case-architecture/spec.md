# backend-use-case-architecture Specification

## Purpose

Defines the uniform shape, base hierarchy, identity resolution, and instantiation rules for backend use cases. Use cases are classes with a single `execute` method, share a two-level base hierarchy (`UseCase` / `AuthedUseCase`), resolve the acting user server-side only, and are constructed fresh per call by the composition root. This encodes ADR-002, ADR-003, and ADR-006.
