...
Define the single global app shell that wraps all authenticated pages, the responsive navigation (desktop sidebar and mobile bottom nav) driven by a shared nav-config, active-route indication, the signed-in landing destination, and the Analytics and Settings destinations.
