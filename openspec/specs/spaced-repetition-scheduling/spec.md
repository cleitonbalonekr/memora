...
Define the pure `schedule(state, grade, today)` domain function that computes a card's next spaced-repetition state from a grade, including interval growth, ease clamping and capping, the lapse behavior of `again`, and suspension of chronic-failure leech cards.
