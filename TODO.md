[x] - Fix src/app/(app)/decks/page.tsx direct call to listDecks
[x] - Work better on the Dependency Injection (Library)
[x] - Add tests
[x] - /opsx:sync  
[x] - Move the function approach to a class oriented so we can handle dependencies better
[ ] - Improve prompt and LLM structured return
[ ] - Improve cards creation - bulk creation
[ ] - Deploy to Cloudflare
[ ] - User your own API key and model
[ ] - Default prompts for each type of study area (general, language, programming, create your own);
[ ] - Improve UI apparence
[ ] - Option to improve a unique cards using AI - rate limit of 5 per improvemnts per day
[ ] - Search for a free redis service or rate limit implementation
